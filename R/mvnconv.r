...
mvnconv <- function(R, side = 2, target, covtocor = FALSE) {
  ...
  # test if the function is called by itself, or in another function.
  testFun <- try(sys.call(-1))
  
  if(is.null(testFun)) {
    # if the upper test is true then this function is called alone, by itself.
    if(missing(target)) {
    ...
    # if else, then this function is called in another function.
    if(missing(target)) {
      ...
      nameFun <- as.character(testFun)[1]  # the name of the function from which this
  ...
  covs[lower.tri(covs)] <- mvnlookup[match(r, mvnlookup[, 1]), colNum]
  diag(covs) <- mvnlookup[which(mvnlookup[, 1] == 0), colNum]
