stouffer <- function(p, adjust = "none", m, R, size = 10000, seed, type = 2, 
  ...
  if (!missing(m)) {
    ...
    if (adjust == "none" && !missing(R))
      warning("although you specified a correlation matrix with argument R, you chose to assume independence with adjust == 'none'. If you want to account for dependence, please specify an adjustment method. See ?stouffer for details.")
    ...
    if (adjust == "none") {
      ...
      if (!missing(emp.step)) {
        ...
        for (i in 1:length(emp.step$thres)) {
          ...
          testStat.tmp  <- sum(qnorm(p, lower.tail = FALSE)) / sqrt(k)
