...
test_that("invchisq() works correctly under independence.", {
  ...
  expect_equivalent(as.numeric(res$p), 6.92e-08, tolerance = p_tol)
  expect_equivalent(as.numeric(res$statistic), 77.9029, tolerance = stat_tol)
...
test_that("invchisq() works correctly with effective number of tests.", {
  ...
  expect_equivalent(as.numeric(res_nyh$p), 1.26e-07, tolerance = p_tol)
  expect_equivalent(as.numeric(res_nyh$statistic), 74.5158, tolerance = stat_tol)
  
  expect_equivalent(as.numeric(res_lj$p), 2.31e-07, tolerance = p_tol)
  expect_equivalent(as.numeric(res_lj$statistic), 71.1287, tolerance = stat_tol)
  
  expect_equivalent(as.numeric(res_gao$p), 6.92e-08, tolerance = p_tol)
  expect_equivalent(as.numeric(res_gao$statistic), 77.9029, tolerance = stat_tol)
  
  expect_equivalent(as.numeric(res_gal$p), 4.24e-07, tolerance = p_tol)
  expect_equivalent(as.numeric(res_gal$statistic), 67.7417, tolerance = stat_tol)
...
test_that("invchisq() works correctly with empirically-derived null distributions.", {
  ...
  expect_equivalent(as.numeric(res$p), 0.00250, tolerance = p_tol)
  expect_equivalent(as.numeric(res$statistic), 77.9029, tolerance = stat_tol)
  expect_equivalent(as.numeric(res$ci[1]), 0.0016, tolerance = stat_tol)
  expect_equivalent(as.numeric(res$ci[2]), 0.0037, tolerance = stat_tol)
  expect_equivalent(attributes(res$statistic)$df, 23, tolerance = df_tol)
  
  set.seed(1234)
  res <- invchisq(grid2ip.p, adjust = "empirical", R = grid2ip.ld, size = 100000)
  
  expect_equivalent(as.numeric(res$p), 0.00229, tolerance = p_tol)
  expect_equivalent(as.numeric(res$statistic), 77.9029, tolerance = stat_tol)
  expect_equivalent(as.numeric(res$ci[1]), 0.0020, tolerance = stat_tol)
  expect_equivalent(as.numeric(res$ci[2]), 0.0026, tolerance = stat_tol)
  expect_equivalent(attributes(res$statistic)$df, 23, tolerance = df_tol)
  
  set.seed(1234)
  res <- invchisq(grid2ip.p, adjust = "empirical", R = grid2ip.ld, size = 1000000, batchsize = 1000)
  
  expect_equivalent(as.numeric(res$p), 0.002324, tolerance = p_tol)
  expect_equivalent(as.numeric(res$statistic), 77.9029, tolerance = stat_tol)
  expect_equivalent(as.numeric(res$ci[1]), 0.0022, tolerance = stat_tol)
  expect_equivalent(as.numeric(res$ci[2]), 0.0024, tolerance = stat_tol)
  expect_equivalent(attributes(res$statistic)$df, 23, tolerance = df_tol)
  
  set.seed(1234)
  res <- invchisq(grid2ip.p, adjust = "empirical", R = grid2ip.ld, size = c(1000, 10000, 100000), threshold = c(0.10, 0.01))
  
  expect_equivalent(as.numeric(res$p), 0.00269, tolerance = p_tol)
  expect_equivalent(as.numeric(res$statistic), 77.9029, tolerance = stat_tol)
  expect_equivalent(as.numeric(res$ci[1]), 0.0024, tolerance = stat_tol)
  expect_equivalent(as.numeric(res$ci[2]), 0.0030, tolerance = stat_tol)
  expect_equivalent(attributes(res$statistic)$df, 23, tolerance = df_tol)
...
test_that("invchisq() works correctly under multivariate theory.", {
  ...
  expect_equivalent(as.numeric(res1$p), 8.45e-05, tolerance = p_tol)
  expect_equivalent(as.numeric(res1$statistic), 38.4072, tolerance = stat_tol)
  ...
  expect_equivalent(as.numeric(res2$p), 0.001015, tolerance = p_tol)
  expect_equivalent(as.numeric(res2$statistic), 24.9528, tolerance = stat_tol)
