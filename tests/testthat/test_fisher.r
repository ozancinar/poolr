...
test_that("fisher() works correctly with empirically-derived null distributions.", {
  ...
  expect_equivalent(c(res$p), 0.0020998, tolerance = p_tol)
  expect_equivalent(c(res$statistic), 118.2920513, tolerance = stat_tol)
  expect_equivalent(c(res$ci[1]), 0.0013003, tolerance = stat_tol)
  expect_equivalent(c(res$ci[2]), 0.0032080, tolerance = stat_tol)
  expect_equivalent(attributes(res$statistic)$df, 46, tolerance = df_tol)

  set.seed(1234)
  res <- fisher(grid2ip.p, adjust = "empirical", R = grid2ip.ld, size = 100000)

  expect_equivalent(c(res$p), 0.0020300, tolerance = p_tol)
  expect_equivalent(c(res$statistic), 118.2920513, tolerance = stat_tol)
  expect_equivalent(c(res$ci[1]), 0.0017605, tolerance = stat_tol)
  expect_equivalent(c(res$ci[2]), 0.0023289, tolerance = stat_tol)
  expect_equivalent(attributes(res$statistic)$df, 46, tolerance = df_tol)

  set.seed(1234)
  res <- fisher(grid2ip.p, adjust = "empirical", R = grid2ip.ld, size = 1000000, batchsize = 1000)

  expect_equivalent(c(res$p), 0.0020410, tolerance = p_tol)
  expect_equivalent(c(res$statistic), 118.2920513, tolerance = stat_tol)
  expect_equivalent(c(res$ci[1]), 0.0019535, tolerance = stat_tol)
  expect_equivalent(c(res$ci[2]), 0.0021314, tolerance = stat_tol)
  expect_equivalent(attributes(res$statistic)$df, 46, tolerance = df_tol)

  set.seed(1234)
  res <- fisher(grid2ip.p, adjust = "empirical", R = grid2ip.ld, size = c(1000, 10000, 100000), threshold = c(0.10, 0.01))

  expect_equivalent(c(res$p), 0.0024000, tolerance = p_tol)
  expect_equivalent(c(res$statistic), 118.2920513, tolerance = stat_tol)
  expect_equivalent(c(res$ci[1]), 0.0021062, tolerance = stat_tol)
  expect_equivalent(c(res$ci[2]), 0.0027232, tolerance = stat_tol)
  expect_equivalent(attributes(res$statistic)$df, 46, tolerance = df_tol)
