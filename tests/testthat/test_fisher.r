...
test_that("fisher() works correctly with effective number of tests.", {
  ...
  res_che <- fisher(grid2ip.p, adjust = "chen", R = mvnconv(grid2ip.ld, target = "p", cov2cor = TRUE))
  ...
  expect_equivalent(c(res_che$p), 3.008722e-09, tolerance = p_tol)
  expect_equivalent(c(res_che$statistic), 121.9391, tolerance = stat_tol)
