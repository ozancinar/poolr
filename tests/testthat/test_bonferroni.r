...
test_that("bonferroni() works correctly with empirically-derived null distributions.", {
  ...
  expect_equivalent(c(res$p), 0.0493951, tolerance = p_tol)
  expect_equivalent(c(res$statistic), 0.0029770, tolerance = stat_tol)
  expect_equivalent(c(res$ci[1]), 0.0452300, tolerance = stat_tol)
  expect_equivalent(c(res$ci[2]), 0.0538250, tolerance = stat_tol)

  set.seed(1234)
  res <- bonferroni(grid2ip.p, adjust = "empirical", R = grid2ip.ld, size = 100000)

  expect_equivalent(c(res$p), 0.0524495, tolerance = p_tol)
  expect_equivalent(c(res$statistic), 0.0029770, tolerance = stat_tol)
  expect_equivalent(c(res$ci[1]), 0.0510758, tolerance = stat_tol)
  expect_equivalent(c(res$ci[2]), 0.0538492, tolerance = stat_tol)

  set.seed(1234)
  res <- bonferroni(grid2ip.p, adjust = "empirical", R = grid2ip.ld, size = 1000000, batchsize = 1000)

  expect_equivalent(c(res$p), 0.0513779, tolerance = p_tol)
  expect_equivalent(c(res$statistic), 0.0029770, tolerance = stat_tol)
  expect_equivalent(c(res$ci[1]), 0.0509461, tolerance = stat_tol)
  expect_equivalent(c(res$ci[2]), 0.0518124, tolerance = stat_tol)

  set.seed(1234)
  res <- bonferroni(grid2ip.p, adjust = "empirical", R = grid2ip.ld, size = c(1000, 10000, 100000), threshold = c(0.10, 0.01))

  expect_equivalent(c(res$p), 0.0483952, tolerance = p_tol)
  expect_equivalent(c(res$statistic), 0.0029770, tolerance = stat_tol)
  expect_equivalent(c(res$ci[1]), 0.0442713, tolerance = stat_tol)
  expect_equivalent(c(res$ci[2]), 0.0527844, tolerance = stat_tol)
