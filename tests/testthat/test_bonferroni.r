...
test_that("bonferroni() works correctly under independence.", {
  ...
  expect_equivalent(c(res$p), 0.068471, tolerance = p_tol)
...
test_that("bonferroni() works correctly with effective number of tests.", {
  ...
  expect_equivalent(c(res_nyh$p), 0.065494, tolerance = p_tol)
  expect_equivalent(c(res_nyh$statistic), 0.0030, tolerance = stat_tol)

  expect_equivalent(c(res_lj$p), 0.062517, tolerance = p_tol)
  expect_equivalent(c(res_lj$statistic), 0.0030, tolerance = stat_tol)

  expect_equivalent(c(res_gao$p), 0.068471, tolerance = p_tol)
  expect_equivalent(c(res_gao$statistic), 0.0030, tolerance = stat_tol)

  expect_equivalent(c(res_gal$p), 0.05954, tolerance = p_tol)
