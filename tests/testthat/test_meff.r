...
test_that("meff() works correctly.", {
  ...
  m_che1 <- meff(R = mvnconv(grid2ip.ld, side = 1, target = "p", cov2cor = TRUE), method = "chen")
  ...
  expect_equivalent(m_che1, 18, tolerance = m_tol)
  ...
  m_che2 <- meff(R = mvnconv(grid2ip.ld, side = 2, target = "p", cov2cor = TRUE), method = "chen")
  ...
  expect_equivalent(m_che2, 22, tolerance = m_tol)
